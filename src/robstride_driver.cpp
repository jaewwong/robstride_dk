...
{
    if (!init_socket()) {
        throw std::runtime_error("Failed to init CAN socket on " + iface);
    }
}
...
ReceiveResult RobStrideMotor::receive(double timeout_sec)
{
    ...
    if (nbytes <= 0) {
        return std::nullopt;   // timeout or fail
    }
    ...
    error_code = static_cast<uint8_t>((can_id >> 16) & 0x3F);
    pattern    = static_cast<uint8_t>((can_id >> 22) & 0x03);
...
bool RobStrideMotor::init_socket()
{
...
bool RobStrideMotor::receive_status_frame()
{
    ReceiveResult result;

    try {
        // timeout_sec = 0 → 블로킹 recv, 필요하면 나중에 timeout 붙여도 됨
        result = receive(0.0);
    }
    catch (const std::exception &e) {
        std::cerr << "[RobStrideMotor] receive() exception: " << e.what() << std::endl;
        return false;
    }

    // 타임아웃 / 실패 등
    if (!result) {
        // std::cerr << "[RobStrideMotor] no frame received" << std::endl;
        return false;
    }
    ...
    if (data.size() < 8) {
        std::cerr << "[RobStrideMotor] data.size() < 8, ignore frame" << std::endl;
        return false;
    }
    ...
    {
        // 0~7 바이트: position / velocity / torque / temperature
        ...
        const auto & op = ACTUATOR_OPERATION_MAPPING.at(static_cast<ActuatorType>(actuator_type));
    ...
    {
        params.data  = static_cast<uint8_t>(data[4]);
        params.index = 0x7005;

        uint16_t idx = (data[1] << 8) | data[0];

        for (int index_num = 0; index_num <= 13; index_num++)
        {
            if (idx != Index_List[index_num]) continue;

            switch(index_num)
            {
                case 0:
                    drw.run_mode.data = static_cast<uint8_t>(data[4]);
    ...
    {
        // 우리가 안 쓰는 프레임 타입이면 그냥 무시
        // std::cerr << "[RobStrideMotor] unknown communication_type=" << (int)communication_type << std::endl;
        return false;
    }
}
